...
function numericQuantity(qty) {
    ...
    var sQty = ("" + qty).replace(vulgarFractionsRegex, function (m, vf) { return vulgarFractionsCharMap[vf]; });
    /**
     *                    Regex captures
     *
     *  +=====+====================+========================+
    ...
    var captureGroup1 = ar[1], captureGroup2 = ar[2];
    ...
    if (captureGroup2.search(/^\./) !== -1) {
        ...
        var numerator = parseFloat(captureGroup2);
        finalResult += Math.round(numerator * 1000) / 1000;
    }
    else if (captureGroup2.search(/^\s*\//) !== -1) {
        // If the first non-space char is "/" it's a pure fraction (e.g. "1/2")
        var numerator = parseInt(captureGroup1);
        var denominator = parseInt(captureGroup2.replace('/', ''));
        finalResult = Math.round((numerator * 1000) / denominator) / 1000;
    }
    else {
        // Otherwise it's a mixed fraction (e.g. "1 2/3")
        var fractionArray = captureGroup2.split('/');
